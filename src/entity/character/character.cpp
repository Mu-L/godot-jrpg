...
namespace rl
{
    ...
    void Character::_ready() {
        ...
        m_firing_point = gdcast<godot::Marker2D>(this->find_child(name::character::firing_pt, true, false));
        ...
        if (m_character_controller != nullptr) {
            signal<event::character_move>::connect<CharacterController>(m_character_controller) <=> signal_callback(this, on_character_movement);
            signal<event::character_rotate>::connect<CharacterController>(m_character_controller) <=> signal_callback(this, on_character_rotate);
            signal<event::character_shoot>::connect<CharacterController>(m_character_controller) <=> signal_callback(this, on_character_shoot);
        }
    }
    ...
    void Character::on_character_movement(godot::Vector2 movement_velocity, double delta_time) {
        const double increment = m_movement_friction * delta_time;
        godot::Vector2 velocity{ this->get_velocity().lerp(movement_velocity, static_cast<float>(increment)) };
        velocity = velocity.clamp({ -1.0, -1.0 }, { 1.0, 1.0 });
        this->translate(velocity * static_cast<float>(this->get_movement_speed()) * static_cast<float>(delta_time));
    ...
    void Character::on_character_rotate(double rotation_angle, double delta_time) {
        const double smoothed_angle = godot::Math::lerp_angle(static_cast<double>(this->get_rotation()), rotation_angle, m_rotation_speed * delta_time);
        this->set_rotation(static_cast<float>(smoothed_angle));
    }
