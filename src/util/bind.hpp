...
namespace rl::inline utils {
    ...
    struct method : public function_traits<decltype(Method)> {
        ...
        static constexpr void bind(std::string_view&& func_name) {
            ...
            if constexpr (tup_size == 0)
                ...
                std::apply(
                    [&](auto&&... args) {
                        godot::ClassDB::bind_method(godot::D_METHOD(func_name.data()), Method, args...);
                    },
                    func_args);
            }
        }
    ...
    struct callback_func : public function_traits<decltype(Function)> {
        ...
        static void bind(std::string_view&& func_name) {
            ...
            if constexpr (tup_size == 0)
                godot::ClassDB::bind_static_method(class_name, godot::D_METHOD(func_name.data()), Function);
            else {
                const typename traits_t::arg_types_nocvref func_args{};
                ...
                std::apply(
                    [&](auto&&... args) {
                        godot::ClassDB::bind_static_method(class_name, godot::D_METHOD(func_name.data()), Function, args...);
                    },
                    func_args);
            }
        }
